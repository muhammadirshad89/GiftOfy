...
import { OCC, STY, TPL } from '../data/content.js';
...
const need = () => { if (!isConfigured || !supabase) throw new WishError(UNAVAILABLE, 'unavailable'); };
...
function toRow(w) {
  const row = {
    ...
    design: TPL.some((t) => t.id === w.t && !t.premium) ? w.t : 'aurora',
...
const fromRow = (r) => ({
  ...
  h: !!r.surprise_mode, t: TPL.some((t) => t.id === r.design && !t.premium) ? r.design : 'aurora',
...
export async function getWishByShortId(id) {
  ...
  need();
  const { data, error, status } = await supabase.rpc('get_wish', { p_short_id: id });
  if (import.meta.env.DEV) { // dev-only trace to tell "RPC failed" from "RPC returned no row"
    console.debug('[GiftOfy] get_wish', { id, status, rows: Array.isArray(data) ? data.length : data ? 1 : 0, error: error && { code: error.code, message: error.message } });
  }
